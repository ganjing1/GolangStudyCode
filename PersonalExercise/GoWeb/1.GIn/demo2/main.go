...
func main() {
	...
	*/
	...
	/*客户端可以通过访问 /s/xxx 的 Routine 来获取 static 目录下的 xxx 文件*/
	r.Static("/s", "F:\\Users\\gj\\gostudy\\PersonalExercise\\GoWeb\\1.GIn\\demo2\\static")

	/*调用了 r.GET 方法，传入了两个参数 “/1.demo1” 和 Hello，
	这个方法会将第一个参数指定的 Routine 路径和第二个参数指定的处理函数绑定起来，也就是说，
	当客户端发送 GET 请求到 /1.demo1 时，会执行 Hello 函数，并返回相应的 HTML 内容。*/
	r.GET("/1.demo1", myFunc.Hello)     //对字符串进行渲染
